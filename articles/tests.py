...
class ArticleTest(TestCase):
	...
	def setup(self):
		...
		photoMock = open(media/images/gnome.jpeg)
		self.post = Article.objects.create(category="Flats",condition="Used",owner_name="test",owner_address="test address",description="test description",location="test location",photo=photoMock)
	...
	def test_article_details(self):
		...
		false_response = self.client.get('/100/')

		self.assertEqual(response.status_code, 200)
		self.assertEqual(false_response.status_code, 404)
